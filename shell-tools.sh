...
install_shell_tools() {
    ...
    print_semiheader "Installing shell-tools" "$COLOR_TERNARY" "" "center"
...
install_packages() {
    ...
    # Actualiza el sistema si no se pasa el parámetro 'preupdate'
    if [ -z "$preupdate" ]; then
        print_semiheader "Updating system"

        case "$SYSTEM_PACKAGE_MANAGER" in
        "pacman") exec_command "sudo pacman -Syu --noconfirm" ;;
        "apt") exec_command "sudo apt-get update && sudo apt-get upgrade" ;;
        "pkg") exec_command "pkg update && pkg upgrade" ;;
        "homebrew") exec_command "brew update && brew upgrade" ;;
        *)
    ...
    case "$SYSTEM_PACKAGE_MANAGER" in
    "pacman") manager_command="sudo pacman -S " ;;
    "apt") manager_command="sudo apt-get install -qq -y --install-recommends " ;;
    "pkg") manager_command="pkg install -y " ;;
    "homebrew") manager_command="brew install -y " ;;
    *)
        echo "Unsupported package manager: $SYSTEM_PACKAGE_MANAGER"
        exit 1
        ;;
    ...
    for package in "${list_packages[@]}"; do
        ...
        exec_command "$manager_command $package"
...
set_terminal_config() {
    case "$(uname -s)" in
    Darwin) SYSTEM_PACKAGE_MANAGER="homebrew" ;;
    Linux)
    ...
    if [ -f /etc/os-release ]; then
        ...
        ID=$(echo -n $(lsb_release -i | awk -F: '{print $2}' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//'))
        VERSION_CODENAME=$(echo -n $(lsb_release -c | awk -F: '{print $2}' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//'))
    else
    ...
    update_config "SYSTEM_PACKAGE_MANAGER" "$SYSTEM_PACKAGE_MANAGER" "$SHELL_TOOLS_CONFIG"
